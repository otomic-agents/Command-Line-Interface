...
while true; do
    ...
    if [ $current_time -ge $break_start_time ] && [ $current_time -lt $break_end_time ]; then
        ...
        node bin/dev.js monkey -i 3600-7200 -r https://5b4522f4.mariansopsoraj.myterminus.com -a 1000-1000 -p $PRIVATE_KEY -s $SOLANA_PRIVATE_KEY -t "succeed" -C true -n mainnet -T 0x28623BF8E872FFAE6e2955a176dFCd10B97a09b3 -S 0x5c48523b82474e6a95e2fd4afc64deb470d1239d997d38ea332227eef84107dd -w http://n8n.edge-dev.xyz/webhook/f8b3f611-89b9-4128-8e93-b50ff7003530 -b "614-0x0000000000000000000000000000000000000000--->9006-0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82" -c "{\"polygon\": \"https://polygon-rpc.com\"}" -l "ballgcolin.myterminus.com" -d true >> "$log_file" 2>&1
